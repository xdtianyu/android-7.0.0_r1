...
UINT16 ZGen_2Phase_Out_Marshal(ZGen_2Phase_Out* source,
  ...
  total_size += TPM2B_ECC_POINT_Marshal(&source->outZ1, buffer, size);
  total_size += TPM2B_ECC_POINT_Marshal(&source->outZ2, buffer, size);
...
TPM_RC ZGen_2Phase_In_Unmarshal(ZGen_2Phase_In* target,
  ...
  target->keyA = request_handles[0];
  // Unmarshal request parameters.
  result = TPM2B_ECC_POINT_Unmarshal(&target->inQsB, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPM2B_ECC_POINT_Unmarshal(&target->inQeB, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result =
      TPMI_ECC_KEY_EXCHANGE_Unmarshal(&target->inScheme, buffer, size, FALSE);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = UINT16_Unmarshal(&target->counter, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_ZGen_2Phase(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result =
      ZGen_2Phase_In_Unmarshal(&in, request_handles, request_parameter_buffer,
