...
UINT16 NV_ReadPublic_Out_Marshal(NV_ReadPublic_Out* source,
  ...
  total_size += TPM2B_NV_PUBLIC_Marshal(&source->nvPublic, buffer, size);
  total_size += TPM2B_NAME_Marshal(&source->nvName, buffer, size);
...
TPM_RC Exec_NV_ReadPublic(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result =
      NV_ReadPublic_In_Unmarshal(&in, request_handles, request_parameter_buffer,
