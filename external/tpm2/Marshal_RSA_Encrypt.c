...
UINT16 RSA_Encrypt_Out_Marshal(RSA_Encrypt_Out* source,
  ...
  total_size += TPM2B_PUBLIC_KEY_RSA_Marshal(&source->outData, buffer, size);
...
TPM_RC RSA_Encrypt_In_Unmarshal(RSA_Encrypt_In* target,
  ...
  result = TPM2B_PUBLIC_KEY_RSA_Unmarshal(&target->message, buffer, size);
...
TPM_RC Exec_RSA_Encrypt(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result =
      RSA_Encrypt_In_Unmarshal(&in, request_handles, request_parameter_buffer,
