...
TPM_RC PolicyDuplicationSelect_In_Unmarshal(PolicyDuplicationSelect_In* target,
  ...
  result = TPM2B_NAME_Unmarshal(&target->newParentName, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPMI_YES_NO_Unmarshal(&target->includeObject, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_PolicyDuplicationSelect(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = PolicyDuplicationSelect_In_Unmarshal(&in, request_handles,
