...
UINT16 PolicyGetDigest_Out_Marshal(PolicyGetDigest_Out* source,
  ...
  total_size += TPM2B_DIGEST_Marshal(&source->policyDigest, buffer, size);
...
TPM_RC PolicyGetDigest_In_Unmarshal(PolicyGetDigest_In* target,
...
TPM_RC Exec_PolicyGetDigest(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = PolicyGetDigest_In_Unmarshal(&in, request_handles,
