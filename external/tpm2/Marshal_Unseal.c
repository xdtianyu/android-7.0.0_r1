...
UINT16 Unseal_Out_Marshal(Unseal_Out* source,
  ...
  total_size += TPM2B_SENSITIVE_DATA_Marshal(&source->outData, buffer, size);
...
TPM_RC Unseal_In_Unmarshal(Unseal_In* target,
...
TPM_RC Exec_Unseal(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = Unseal_In_Unmarshal(&in, request_handles, request_parameter_buffer,
...
// Marshal output structure to global response buffer.
  ...
  bytes_marshalled =
      Unseal_Out_Marshal(&out, tag, &response_buffer, &response_buffer_size);
