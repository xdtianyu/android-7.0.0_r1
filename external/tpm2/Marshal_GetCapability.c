...
UINT16 GetCapability_Out_Marshal(GetCapability_Out* source,
  ...
  total_size += TPMI_YES_NO_Marshal(&source->moreData, buffer, size);
  total_size +=
      TPMS_CAPABILITY_DATA_Marshal(&source->capabilityData, buffer, size);
...
TPM_RC GetCapability_In_Unmarshal(GetCapability_In* target,
  ...
  result = UINT32_Unmarshal(&target->property, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = UINT32_Unmarshal(&target->propertyCount, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_GetCapability(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result =
      GetCapability_In_Unmarshal(&in, request_handles, request_parameter_buffer,
