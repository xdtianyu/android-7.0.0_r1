...
UINT16 VerifySignature_Out_Marshal(VerifySignature_Out* source,
  ...
  total_size += TPMT_TK_VERIFIED_Marshal(&source->validation, buffer, size);
...
TPM_RC VerifySignature_In_Unmarshal(VerifySignature_In* target,
  ...
  result = TPMT_SIGNATURE_Unmarshal(&target->signature, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_VerifySignature(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = VerifySignature_In_Unmarshal(&in, request_handles,
