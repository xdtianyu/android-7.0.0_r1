...
UINT16 FieldUpgradeData_Out_Marshal(FieldUpgradeData_Out* source,
  ...
  total_size += TPMT_HA_Marshal(&source->nextDigest, buffer, size);
  total_size += TPMT_HA_Marshal(&source->firstDigest, buffer, size);
...
TPM_RC FieldUpgradeData_In_Unmarshal(FieldUpgradeData_In* target,
  ...
  result = TPM2B_MAX_BUFFER_Unmarshal(&target->fuData, buffer, size);
...
TPM_RC Exec_FieldUpgradeData(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = FieldUpgradeData_In_Unmarshal(&in, request_handles,
