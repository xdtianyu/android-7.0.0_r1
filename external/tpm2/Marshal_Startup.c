...
TPM_RC Startup_In_Unmarshal(Startup_In* target,
                            TPM_HANDLE request_handles[],
                            BYTE** buffer,
                            INT32* size) {
  TPM_RC result = TPM_RC_SUCCESS;
  // Unmarshal request parameters.
  result = TPM_SU_Unmarshal(&target->startupType, buffer, size);
...
TPM_RC Exec_Startup(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = Startup_In_Unmarshal(&in, request_handles, request_parameter_buffer,
