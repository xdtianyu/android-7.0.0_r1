...
TPM_RC Sign_In_Unmarshal(Sign_In* target,
  ...
  result = TPMT_TK_HASHCHECK_Unmarshal(&target->validation, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_Sign(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = Sign_In_Unmarshal(&in, request_handles, request_parameter_buffer,
...
// Marshal output structure to global response buffer.
  ...
  bytes_marshalled =
      Sign_Out_Marshal(&out, tag, &response_buffer, &response_buffer_size);
