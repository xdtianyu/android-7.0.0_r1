...
UINT16 EncryptDecrypt_Out_Marshal(EncryptDecrypt_Out* source,
  ...
  total_size += TPM2B_MAX_BUFFER_Marshal(&source->outData, buffer, size);
  total_size += TPM2B_IV_Marshal(&source->ivOut, buffer, size);
...
TPM_RC EncryptDecrypt_In_Unmarshal(EncryptDecrypt_In* target,
  ...
  result = TPMI_YES_NO_Unmarshal(&target->decrypt, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPMI_ALG_SYM_MODE_Unmarshal(&target->mode, buffer, size, TRUE);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPM2B_IV_Unmarshal(&target->ivIn, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPM2B_MAX_BUFFER_Unmarshal(&target->inData, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_EncryptDecrypt(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = EncryptDecrypt_In_Unmarshal(&in, request_handles,
