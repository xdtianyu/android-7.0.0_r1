...
UINT16 ECDH_ZGen_Out_Marshal(ECDH_ZGen_Out* source,
  ...
  total_size += TPM2B_ECC_POINT_Marshal(&source->outPoint, buffer, size);
...
TPM_RC ECDH_ZGen_In_Unmarshal(ECDH_ZGen_In* target,
  ...
  result = TPM2B_ECC_POINT_Unmarshal(&target->inPoint, buffer, size);
...
TPM_RC Exec_ECDH_ZGen(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result =
      ECDH_ZGen_In_Unmarshal(&in, request_handles, request_parameter_buffer,
...
// Marshal output structure to global response buffer.
  ...
  bytes_marshalled =
      ECDH_ZGen_Out_Marshal(&out, tag, &response_buffer, &response_buffer_size);
