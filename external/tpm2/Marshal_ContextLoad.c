...
UINT16 ContextLoad_Out_Marshal(ContextLoad_Out* source,
  ...
  total_size += TPMI_DH_CONTEXT_Marshal(&source->loadedHandle, buffer, size);
...
TPM_RC ContextLoad_In_Unmarshal(ContextLoad_In* target,
  ...
  result = TPMS_CONTEXT_Unmarshal(&target->context, buffer, size);
...
TPM_RC Exec_ContextLoad(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result =
      ContextLoad_In_Unmarshal(&in, request_handles, request_parameter_buffer,
