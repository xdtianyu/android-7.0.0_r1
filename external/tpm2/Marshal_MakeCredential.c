...
UINT16 MakeCredential_Out_Marshal(MakeCredential_Out* source,
  ...
  total_size += TPM2B_ID_OBJECT_Marshal(&source->credentialBlob, buffer, size);
  total_size += TPM2B_ENCRYPTED_SECRET_Marshal(&source->secret, buffer, size);
...
TPM_RC MakeCredential_In_Unmarshal(MakeCredential_In* target,
  ...
  result = TPM2B_DIGEST_Unmarshal(&target->credential, buffer, size);
...
TPM_RC Exec_MakeCredential(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = MakeCredential_In_Unmarshal(&in, request_handles,
