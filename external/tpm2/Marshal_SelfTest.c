...
TPM_RC SelfTest_In_Unmarshal(SelfTest_In* target,
                             TPM_HANDLE request_handles[],
                             BYTE** buffer,
                             INT32* size) {
  TPM_RC result = TPM_RC_SUCCESS;
  // Unmarshal request parameters.
  result = TPMI_YES_NO_Unmarshal(&target->fullTest, buffer, size);
...
TPM_RC Exec_SelfTest(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = SelfTest_In_Unmarshal(&in, request_handles, request_parameter_buffer,
