...
UINT16 ECDH_KeyGen_Out_Marshal(ECDH_KeyGen_Out* source,
  ...
  total_size += TPM2B_ECC_POINT_Marshal(&source->zPoint, buffer, size);
  total_size += TPM2B_ECC_POINT_Marshal(&source->pubPoint, buffer, size);
...
TPM_RC Exec_ECDH_KeyGen(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result =
      ECDH_KeyGen_In_Unmarshal(&in, request_handles, request_parameter_buffer,
