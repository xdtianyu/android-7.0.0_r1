...
TPM_RC FieldUpgradeStart_In_Unmarshal(FieldUpgradeStart_In* target,
                                      TPM_HANDLE request_handles[],
                                      BYTE** buffer,
                                      INT32* size) {
  TPM_RC result = TPM_RC_SUCCESS;
  // Get request handles from request_handles array.
  target->authorization = request_handles[0];
  target->keyHandle = request_handles[1];
  // Unmarshal request parameters.
  result = TPM2B_DIGEST_Unmarshal(&target->fuDigest, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPMT_SIGNATURE_Unmarshal(&target->manifestSignature, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_FieldUpgradeStart(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = FieldUpgradeStart_In_Unmarshal(&in, request_handles,
