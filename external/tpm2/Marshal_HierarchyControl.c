...
TPM_RC HierarchyControl_In_Unmarshal(HierarchyControl_In* target,
  ...
  result = TPMI_RH_ENABLES_Unmarshal(&target->enable, buffer, size, FALSE);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPMI_YES_NO_Unmarshal(&target->state, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_HierarchyControl(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = HierarchyControl_In_Unmarshal(&in, request_handles,
