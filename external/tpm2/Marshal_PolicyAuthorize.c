...
TPM_RC PolicyAuthorize_In_Unmarshal(PolicyAuthorize_In* target,
  ...
  result = TPM2B_DIGEST_Unmarshal(&target->approvedPolicy, buffer, size);
  ...
  result = TPM2B_NAME_Unmarshal(&target->keySign, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
  result = TPMT_TK_VERIFIED_Unmarshal(&target->checkTicket, buffer, size);
  if (result != TPM_RC_SUCCESS) {
    return result;
  }
...
TPM_RC Exec_PolicyAuthorize(TPMI_ST_COMMAND_TAG tag,
  ...
  *response_parameter_buffer_size = 0;
  ...
  result = PolicyAuthorize_In_Unmarshal(&in, request_handles,
